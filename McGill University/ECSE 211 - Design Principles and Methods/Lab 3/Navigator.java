...
public class Navigator extends Thread {
	...
	public Navigator(Odometer odometer, EV3LargeRegulatedMotor leftMotor, EV3LargeRegulatedMotor rightMotor,
			int waypoints[][], EV3LargeRegulatedMotor usMotor, UltrasonicPoller usPoller, boolean avoidance,
			double track) {
	...
	public void travelTo(double x, double y) {
		...
		while (Math.hypot(currentX - x, currentY - y) > DISTANCE_ERROR) {
			...
			if (avoidance) {
				...
				rightMotor.rotate(convertDistance(Lab3.WHEEL_RADIUS, distance), true);
				...
				moving_loop: while (isMoving()) {
					...
					if (usPoller.getDistance() < WALL_DISTANCE) {
						...
						leftMotor.rotate(-convertAngle(Lab3.WHEEL_RADIUS, track, RIGHT_ANGLE), true); // 1
																										// (1.57)
						rightMotor.rotate(convertAngle(Lab3.WHEEL_RADIUS, track, RIGHT_ANGLE), false); // 1
																										// (1.57)
						...
						wallfollower_loop: while (true) {
							...
							int bang_bang_error = usPoller.getDistance() - BAND_CENTER;
	...
	public void turnTo(double theta, double ctheta) {
		...
		leftMotor.rotate(convertAngle(Lab3.WHEEL_RADIUS, track, changeTheta), true);
		rightMotor.rotate(-convertAngle(Lab3.WHEEL_RADIUS, track, changeTheta), false);
		currentTheta = odometer.getTheta();
